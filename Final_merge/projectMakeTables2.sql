...
create table games(
    ...
    p_name varchar (30),
    primary key(title),
    foreign key(p_name) references publisher(p_name) ON DELETE CASCADE
);
create table game_modes(
    ...
    foreign key(title) references games(title) ON DELETE CASCADE
);
create table system(
...
create table online_features(
    ...
    foreign key(brand_name) references system(brand_name) ON DELETE CASCADE
);
...
SOURCE insertpublisher.sql;
SOURCE insertlocation.sql;
SOURCE insertgames.sql;
SOURCE insertgame_modes.sql;
SOURCE insertsystem.sql;
SOURCE insertonline_features.sql;
